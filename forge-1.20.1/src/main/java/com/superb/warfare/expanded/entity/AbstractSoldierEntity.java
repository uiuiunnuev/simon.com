...
public abstract class AbstractSoldierEntity extends Monster implements RangedAttackMob {
    ...
    protected void registerGoals() {
        ...
        // We will configure a custom goal for firing / attacking in subclasses
        this.goalSelector.addGoal(5, new WaterAvoidingRandomStrollGoal(this, 0.6D) {
            @Override
            public boolean canUse() {
                // Walk slower/only during patrol
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(STATE, STATE_PATROL);
    }
    ...
    public void triggerAlert(LivingEntity enemy) {
        if (!this.level().isClientSide) {
            ...
            // Propagate alarm to nearby friendly soldiers
        ...
        if (!this.level().isClientSide) {
            if (getSoldierState() == STATE_ALERT) {
                if (this.getTarget() == null || !this.getTarget().isAlive()) {
                    ...
                    this.alertTicks = 300; // Reset alert timer as long as enemy is present
                }
            }
        }
    }
    ...
    protected void populateDefaultEquipmentSlots(DifficultyInstance difficulty) {
        ...
        try {
            ResourceLocation weaponLoc = new ResourceLocation(gunId);
            if (ForgeRegistries.ITEMS.containsKey(weaponLoc)) {
                weaponStack = new ItemStack(ForgeRegistries.ITEMS.getValue(weaponLoc));
            }
        } catch (Exception ignored) {}

        if (weaponStack.isEmpty()) {
            // Fallback weapon if TaCZ is not installed
